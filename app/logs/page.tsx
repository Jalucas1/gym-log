...
export default function LogsPage() {
  const [activeTab, setActiveTab] = useState<"cleaning" | "tanning">("cleaning");
  ...
  useEffect(() => {
    async function loadLogs() {
      ...
      setCleaningLogs((cleaningData || []) as CleaningLog[]);
    ...
    <AuthGuard>
      <AppFrame>
        ...
        {loading ? (
                ...
                <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                  <div>
                    ...
                    <h2 className="mt-1 text-xl font-black text-black">
                      {log.task_name || "Cleaning task"}
                    </h2>
                    <p className="mt-1 text-sm font-bold text-black/60">
                      {log.employee_initials ? `${log.employee_initials} — ` : ""}
          ...
          <section className="space-y-4">
            {tanningLogs.length === 0 && <EmptyState message="No tanning logs yet." />}
